function grabJSON (){  //This function grabs the information from the JSON file and determines how many people are on each baby step and displays the appropriate information on the webpage.
	$.getJSON("assets/javascripts/baby-steps.json", function(response){
			...
			if (names.length === 0) {
				...
				$(".replace-this").replaceWith("<p class='replace-this'><a>" + names[0].firstName +" "+ names[0].lastName + "</a> is also on Baby Step " +$onBabyStepInt+".</p>");
			} else if (names.length === 2) {
				$(".replace-this").replaceWith("<p class='replace-this'><a>" + names[0].firstName +" "+ names[0].lastName + "</a> and <a> " + names[1].firstName +" "+ names[1].lastName + "</a> are also on Baby Step " +$onBabyStepInt+".</p>");
			} else if (names.length === 3) {
				$(".replace-this").replaceWith("<p class='replace-this'><a>" + names[0].firstName +" "+ names[0].lastName + "</a>, <a>" + names[1].firstName +" "+ names[1].lastName +"</a> and 1 other friend are also on Baby Step "+$onBabyStepInt+".</p>");
			} else {
				$(".replace-this").replaceWith("<p class='replace-this'><a>" + names[0].firstName +" "+ names[0].lastName + "</a>, <a>" + names[1].firstName +" "+ names[1].lastName +"</a> and "+(names.length - 2)+" other friends are also on Baby Step "+$onBabyStepInt+".</p>");
			}
	});
}
